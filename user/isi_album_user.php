            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                            ...
                            <div class="overflow-auto" style="max-height: 300px;"> <!-- Atur ketinggian maksimum dan tambahkan overflow auto -->
                                ...
                                while ($komentar = mysqli_fetch_assoc($query_komentar)) :
                                    ...
                                    <div style='display:flex;justify-content:space-between;flex-direction:row'>
                                        <div>
                                            <p  style='word-wrap: break-word;'><strong><?= $komentar['username'] ?> :</strong> <?= $komentar['comment_text']?> </p>
                                        </div>
                                        <?php if($komentar['user_id'] === $_SESSION['user_id']): ?>
                                        <div>
                                            <i id='btn-dropdown<?=$komentar['comments_id'] ?>' class="fa-solid fa-ellipsis-vertical" style='cursor:pointer'></i>
                                            <div id='dropdown<?=$komentar['comments_id'] ?>' style='background-color:white;position:absolute;padding:12px;border:1px solid #303030;display:none'>
                                                <form method='post' action='../config/proses_komentar.php'>
                                                    <input type='hidden' name='komenid' value='<?=$komentar['comments_id'] ?>'/>
                                                    <button name='hapuskomen' style='border:none;background-color:white'><p class='text-danger'>Hapus</p></button>
                                                </form>
                                                <!-- <button type="button" class="btn btn-primary dropdown-item" data-toggle="modal" data-target="#editKomentarModal" data-comment-id="<?php echo $komentar['comments_id']; ?>">Edit</button> -->
                                                <!-- <a href='#'    data-comment-id="<?php echo $komentar['comments_id']; ?>"  data-bs-toggle="modal" data-bs-target="#editkomentar<?php echo $data['photo_id']; ?>" style=''><p class='text-primary'>Edit</p></a> -->
                                            </div>
                                        </div>
                                        <?php endif ?>
                                    </div>
                                    <form method='post' action='../config/hapus_komentar.php'>
                        ...
                        <a href='#' data-bs-toggle="modal" class="btn btn-danger" data-bs-target="#laporan<?php echo $data['photo_id']; ?>">Laporkan Postingan</a>
                    </div>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="laporan<?php echo $data['photo_id']; ?>" tabindex="-1" aria-labelledby="komentarModalLabel<?php echo $data['photo_id']; ?>" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="komentarModalLabel<?php echo $data['photo_id']; ?>">Laporkan Postingan</h5> <!-- Menampilkan judul foto -->
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <form method='post' action='../config/tambah_laporan.php'>
                            <textarea name='isi_laporan' style='width:100%;height:10em' resize='none' class='mb-2'></textarea>
                            <input type='hidden' name='foto_id' value='<?= $data['photo_id'] ?>'/>
...
<div class="modal fade" id="editkomentar<?php echo $data['photo_id']; ?>" tabindex="-1" aria-labelledby="komentarModalLabel<?php echo $data['photo_id']; ?>" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="komentarModalLabel<?php echo $data['photo_id']; ?>">Edit Komentar</h5> <!-- Menampilkan judul foto -->
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <!-- Deskripsi Foto -->
                    <form method='post' action='../config/proses_komentar'>
                    <div class="col-md-12 mb-3">
                            <textarea name='isikomentar' class='form-control' style='width:100%;height:120px'></textarea>
                            <input type="hidden" name="photo_id" value="<?php echo $data['photo_id']; ?>">
                            <input type="number" id="editKomentarCommentId" name="commentId">
                        </div>
                    <div class="col-md-12">
                            <button type='submit' name='editkomentar' class='btn btn-primary'>kirim</button>
...
<script>
     document.addEventListener("DOMContentLoaded", function() {
        var btnDropdowns = document.querySelectorAll('[id^="btn-dropdown"]');
        
        btnDropdowns.forEach(function(btnDropdown) {
            btnDropdown.addEventListener("click", function() {
                var commentId = this.id.replace('btn-dropdown', '');
                var dropdown = document.getElementById('dropdown' + commentId);
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
  var editButtons = document.querySelectorAll('[data-bs-target="#editkomentar"]');
  
  editButtons.forEach(function(editButton) {
    editButton.addEventListener("click", function() {
      var commentId = this.id.split('-')[1]; // Extract comments_id from the id attribute of the button
      var editKomentarInput = document.getElementById('editKomentarInput'); // Ambil input edit komentar modal
      editKomentarInput.value = this.dataset.commentText; // Masukkan teks komentar ke dalam input edit komentar modal
      editKomentarInput.dataset.commentId = commentId; // Masukkan comments_id ke dalam atribut data pada input edit komentar modal
    });
  });
});
});
